...
/**
 * Basic link resolver that matches a list of criteria and returns pattern based
 * link URIs.
 * 
...
public class SimpleExternalLinkResolver implements ExternalLinkResolver {
	...
	private String datasetGroup; // used if external link is tied to a specific dataset. NULL if none required
									// (default)
		...
		outer: for (String critField : getCriteria().keySet()) {
			...
			if (fieldContent != null) {
				if (critContent.isEmpty()) {
					matches = true;
					if (!isMatchAllCriteria())
						break;
				} else {
					String[] critValues = critContent.split(",");
					for (String critValue : critValues) {
						if (isExactMatch())
							matches = fieldContent.equals(critValue);
						else
							matches = fieldContent.contains(critValue);
						if (matches && !isMatchAllCriteria() || !matches && isMatchAllCriteria())
							break outer;
					}
				}
			} else {
		...
		if (matches) {
			...
			if (getPatternFields() != null && !getPatternFields().isEmpty()) {
				...
				if (patternValues[0] != null) {
					url = url.formatted(patternValues);
					validationUrl = validationUrl.formatted(patternValues);
				} else {
					url = null;
					validationUrl = null;
				}
			...
			if (datasetGroup != null) {
				...
				String fieldAdress = tableName + ".DatensatzGruppe" + tableName.substring(0, 1).toUpperCase()
	 ...
	 * This determines if all criteria have to be met in order for a link to be
	 * created or just one (default).
	 * 
	 ...
	 * Get exactMatch flag.
	 * 
	 * @return <code>true</code> if exactMatch flag is set, <code>false</code>
	 *         otherwise
	 */
	...
	/**
	 * Set exactMatch flag.
	 * This determines if the values given in the criteria map have to match to the
	 * dataset's
	 * field content perfectly (default) or if the field only needs to contain the
	 ...
	 * The map entries describe properties a dataset has to fulfill in order to be
	 * resolve.
	 * Keys correspond to field names, values to field values.
	 * Values can be comma-separated in order to give more than one criterion for
	 * the same field name.
	 * 
