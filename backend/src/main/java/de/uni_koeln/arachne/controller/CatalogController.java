...
public class CatalogController {
	 ...
	 * Returns a catalogEntry entity which is serialized to JSON. If the given id
	 * does not refer to a
	 ...
	 * @param limit          If full = false then limit restricts the number of
	 *                       direct children to the desired value
	 *                       (-1 for no limit).
	 * @param offset         If full = false and limit > 0 then offset gives an
	 *                       offset into the direct children list.
	 * @return A {@link ResponseEntity} containing the catalog entry or the error
	 *         code on failures.
	 */
	@GetMapping(value = "entry/{catalogEntryId}", produces = CustomMediaType.APPLICATION_JSON_UTF8_VALUE)
	public @ResponseBody ResponseEntity<CatalogEntry> handleGetCatalogEntryRequest(
			@PathVariable final Long catalogEntryId,
	 ...
	 * 
	 * @param catalogEntryId  The catalog id of the catalog the entry shall be added
	 *                        to.
	 * @param newCatalogEntry The catalog entry to add.
	 * @return A {@link ResponseEntity} containing the catalog entry or the
	 *         corresponding error code on failure.
	 */
	@PutMapping(value = "entry/{catalogEntryId}", consumes = CustomMediaType.APPLICATION_JSON_UTF8_VALUE, produces = CustomMediaType.APPLICATION_JSON_UTF8_VALUE)
	...
	/**
	 * Handles HTTP DELETE requests for <code>catalog/entry/{catalogEntryId}</code>.
	 * Deletes
	 ...
	 */
	@DeleteMapping(value = "entry/{catalogEntryId}", produces = CustomMediaType.APPLICATION_JSON_UTF8_VALUE)
	 ...
	 * Returns an HTTP status code 403 if the user is not allowed to modify the
	 * catalog the entries belong to or 400 if
	 * the posted catalog entries are not semantically correct.
	 * 
	 * @param catalogEntries The catalog entries.
	 * @return A {@link ResponseEntity} containing the persisted entry or the
	 *         corresponding error code on failure.
	 */
	@PostMapping(value = "entry", consumes = CustomMediaType.APPLICATION_JSON_UTF8_VALUE, produces = CustomMediaType.APPLICATION_JSON_UTF8_VALUE)
	 ...
	 * that is signed in, serialized to JSON. If the current user does not own any
	 * catalogs an empty List is returned.
	 * If no user is signed in, a 403 error code is returned.
	 * 
	 * @param limit  If full == false then this parameter limits the children of the
	 *               root entry (-1 for no limit).
	 * @param offset If full == false then this parameter is an offset into the
	 *               children of the root entry.
	 * @return A {@link ResponseEntity}
	 */
	@GetMapping(value = "", produces = CustomMediaType.APPLICATION_JSON_UTF8_VALUE)
	 ...
	 * @param limit     If full == false then this parameter limits the children of
	 *                  the root entry (-1 for no limit).
	 * @param offset    If full == false then this parameter is an offset into the
	 *                  children of the root entry.
	 * @return The catalog.
	 */
	 ...
	 * Returns the catalog created and 200 if the action is permitted. Returns null
	 * and 403 if no user is signed in or
	 * the signed in user does not own the catalog to be edited. This method accepts
	 * updates on nested
	 * {@link CatalogEntry} items' fields. and creates nested {@link CatalogEntry}
	 * items if they do not already exist.
	 * It does not automatically delete items, that are missing from the list of
	 * nested {@link CatalogEntry} items.
	 * 
	 * @param catalog     The updated catalog.
	 * @param requestedId The id of the catalog to update.
	 * @return A {@link ResponseEntity} containing the updated catalog as retrieved
	 *         from the DB.
	 */
	@PutMapping(value = "{requestedId}", consumes = CustomMediaType.APPLICATION_JSON_UTF8_VALUE, produces = CustomMediaType.APPLICATION_JSON_UTF8_VALUE)
	public @ResponseBody ResponseEntity<Catalog> handleCatalogUpdateRequest(
			@RequestBody final Catalog catalog,
			@PathVariable final Long requestedId) {
	 ...
	 * Returns the catalog created and 200 if the action is permitted. Returns 403
	 * if no user is signed in and 422 if
	 * the catalog could not be created.
	 * This method creates all nested {@link CatalogEntry} items. Existing primary
	 * id values in nested
	 * {@link CatalogEntry} items are ignored.
	 * 
	 * @param catalog The catalog.
	 * @return A {@link ResponseEntity} containing the catalog as retrieved from the
	 *         DB or a status code on failure.
	 */
	@PostMapping(value = "", consumes = CustomMediaType.APPLICATION_JSON_UTF8_VALUE, produces = CustomMediaType.APPLICATION_JSON_UTF8_VALUE)
	...
	public ResponseEntity<String> handleCatalogDestroyRequest(@PathVariable final Long catalogId) {
	 ...
	 *         {@link CatalogEntryExtended}.
	 */
	@GetMapping(value = "list/{entityId}", produces = CustomMediaType.APPLICATION_JSON_UTF8_VALUE)
