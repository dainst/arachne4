...
/**
 * This class provides functions to find a XML config file by type, extract
 * information based on the XML element from the dataset and grants access to
 * the servlet context. If a class wants to work with the XML config files it
 * should use this class via autowiring or as base class.
 * 
...
public class XmlConfigUtil implements ServletContextAware {
	...
	/**
	 * Convenience method to clear the current XML config document, element,
	 * mandatory contexts and context image descriptors cache.
	 */
	...
	/**
	 * This function handles context elements in the xml config files. It
	 * extracts the content from the dataset following the definitions in the
	 * xml files and returns it as <code>Content</code>. <br>
	...
	public Section getContentFromContext(final Element context, final Namespace namespace, final Dataset dataset,
		...
		if (contextSections == null || contextSections.isEmpty()) {
			...
			if (link != null) {
				...
				// first extracts the start date timestamp and then uses that
				// for comparison
				Collections.sort(tempPlaceList, new Comparator<String>() {
					public int compare(String o1, String o2) {
						...
						// Shuffles entries with date values (indicated by
						// "Ortsangabe von") to the top
						if (index1 == -1 || index2 == -1) {
							...
							String date2;
							try {
								date1 = o1.substring(index1 + 16, o1.indexOf(" ", index1 + 21)); // until
																									// first
																									// space
							...
							String[] dateParts1 = date1.split("\\."); // split
																		// days,
																		// months
																		// and
																		// year
			...
			for (int i = 0; i < dataset.getContextSize(contextType); i++) {
				...
				for (final Element curSection : contextSections) {
					...
					if (subTitleSection != null) {
						final AbstractContent newHeadline = getContentFromSections(subTitleSection,
								context.getNamespace(), dataset.getContext(contextType).getContext(i).getEntity2(),
								lang);
						if (newHeadline != null) {
					...
					// add all child-fields of the current contextSection and
					// retrieve their values
					for (final Element childField : curSection.getChildren("field", context.getNamespace())) {
						addContextFieldToFieldList(childField, context.getNamespace(), fieldList, i, dataset,
								contextType, separator, lang);
					}
					...
					if (subContext != null) {
						addSubContext(localContext, contextType, subContext, subContext.getNamespace(),
								dataset.getContext(contextType).getContext(i).getEntity2(), lang);
						// addSubContext(localContext, contextType, childField,
						// namespace ,
						// dataset.getContext(contextType).getContext(i).getEntity2(),
						// lang);
					}
	...
	/**
	 * This function handles sections in the xml config files. It extracts the
	 * content from the dataset following the definitions in the xml files and
	 * returns it as <code>Content</code>. <br>
	 * The validity of the xml file is not checked!!!
	 * 
	 * @param section
	...
	public AbstractContent getContentFromSections(final Element section, final Namespace namespace,
			final Dataset dataset, final String lang) {
		try {
			final Section result = new Section();
	...
	/**
	 * Methode retrieves ContextImageDescriptor-instances. If these instances
	 * have beeen requested before, the method returns cached instances, else it
	...
	/**
	 * This method retrieves a XML document based on the type. It tries to find
	 * the corresponding XML file, parses it and constructs the DOM. Already
	...
	/**
	 * Returns a list of contextualizers found in the
	 * <code>explicitContextualizers</code> tag of the xml file. If there is a
	 * cached version this is returned else the list is created, cached and then
	 * returned.
	 * 
	 * @param type
	...
	public List<String> getExplicitContextualizers(final String type) {
		...
		if (cachedContextualizers == null) {
			...
			if (contextualizers != null) {
				final List<String> explicitContextualizerTypeList = StrUtils
						.getCommaSeperatedStringAsList(contextualizers.getTextNormalize());
	...
	/**
	 * This method looks up which facets are defined in an XML file describing a
	 * category.
	 * 
	 * @param type
		...
		if (cachedFacets == null) {
			...
			if (document != null) {
				...
				for (final Element element : facetsElements.getChildren()) {
					...
					facetMap.put(element.getAttributeValue("name"),
							new SearchResultFacet(element.getAttributeValue("name"), element.getAttributeValue("group"),
	...
	public SearchResultFacet getFacetInfo(String category, String facetName) {
		...
		if (facetCache.get(category).get(facetName.substring(6)) != null) {
			return facetCache.get(category).get(facetName.substring(6)); // remove
																			// the
																			// "facet_"-prefix
		}
	...
	/**
	 * Returns the content of a field of the dataset as defined inside an
	 * <code>ifEmtpy</code> tag in the XML config file. It is safe to use even
	 * if the passed in <code>Element</code> does not have an
	...
	/**
	 * Returns the content of a field of the dataset as defined inside an
	 * <code>ifEmtpy</code> tag in a <code>Context</code> tag in the XML config
	 * file. It is safe to use even if the passed in <code>Element</code> does
	 ...
	 *         <code>null</code> if no value could be retrieved or the passed in
	 *         <code>Element</code> does not have an
	 *         <code>ifEmpty</code>-Element as a child.
	 */
	public StringBuilder getIfEmptyContext(final Element element, final Namespace namespace, final Dataset dataset,
		...
		if (ifEmptyElement != null) {
			...
			if (key != null && !key.isEmpty()) {
				...
				if (ifEmptyValue == null) {
					result = getIfEmptyContext(ifEmptyElement.getChild("field", namespace), namespace, dataset,
							contextType, index);
				} else {
					result = new StringBuilder(16).append(ifEmptyValue);
				}
	...
	/**
	 * This method returns the list of unique context names that are needed to
	 * retrieve all data for a given category. The list is only created if it is
	 * not cached.
	 * 
	 * @param type
	 *            The category of the parent dataset.
	...
	/**
	 * Returns a list of subCategories that may contain additional information
	 * needed for the dataset (like 'objektkeramik' for 'objekt'). So this
	 * methods handles the subCategories tag in the xml file and caches the
	 * information.
	 * 
	 * @param type
	 *            The category type.
	...
	/**
	 * This method constructs a string list of the cached XML config document
	 * names.
	 * 
	...
	/**
	 * This method constructs a string list of the cached XML include element
	 * names.
	 * 
	...
	/**
	 * This method returns a XML parser to build the DOM. The parser is only set
	 * up once and can be reused.
	 * 
	...
	/**
	 * This function adds the value of a field of a context to the
	 * <code>FieldList</code>. It retrieves the value and formats the output
			...
			if (element.getName().contentEquals("linkField")) {
				try {
					...
					if (StrUtils.isEmptyOrNullOrZero(labelKey)) {
						value = new StringBuilder(32).append("<a href=\"").append(value.toString())
								.append("\"  target=\"_blank\">").append(value.toString()).append("</a>");
					} else {
	...
	private String addContextCompoundDateFieldToFieldList(final Element element, final Namespace namespace,
			final FieldList fieldList, final int index, final Dataset dataset, final String contextType,
			String separator, final String lang) {
		...
		for (final Element child : element.getChildren()) {
			...
			if (dataset.getFieldFromContext(contextType + child.getAttributeValue("datasource"), index) != null) {
				toAppend.append(
						dataset.getFieldFromContext(contextType + child.getAttributeValue("datasource"), index));
			}
	...
	private void addContextFieldToList(final Element field, final String parentType, final List<String> result,
		...
		if (!StrUtils.isEmptyOrNullOrZero(datasourceValue)) {
			...
			// Exception for objekt-Subgroups e.g. objektkeramik, these are
			// handled specially
			// Add Exception for Fabric / Fabricdescription, Surfacetreatment /
			// SurfacetreatmentAction
			// as they shouldnt be handled as Object-/Sub-Object => the problem
			// is, that the child-type
			// starts with the name of the parent
			if ((!datasourceValue.startsWith(parentType) && !datasourceValue.startsWith("Dataset"))
					|| "surfacetreatment".equals(parentType) || "fabric".equals(parentType)) {
	...
	/**
	 * This function adds the values of all fields of a context to the
	 * <code>FieldList</code>. It retrieves the values and formats the output
	...
	private void addFieldsToFieldList(final List<Element> children, final Namespace namespace,
			final FieldList fieldList, final int index, final Dataset dataset, final String contextType,
			final String separator, final String lang) {
		...
		for (final Element element : children) {
			if (element.getName().equals("field") || element.getName().equals("linkField")) {
				...
				nextSeparator = addContextPlainToFieldList(element, fieldList, index, dataset, contextType,
						nextSeparator, lang);
			}
			nextSeparator = (nextSeparator != null) ? nextSeparator : separator;
		}
	...
	/**
	 * Procedure to add a <code>Field</code> from the dataset to the result
	 * <code>Section</code>.
	 * 
	 * @param dataset
	 *            The current dataset.
	...
	private void addFieldToResult(final Element element, final Namespace namespace, final Section result,
		...
		if (value != null) {
			...
			// TODO find better solution as the previous content may be a
			// section
			// If there are more than one field in this section add the value
			// (incl. separator) to the previous field
			if (result.getContent().isEmpty()) {
	...
	/**
	 * Procedure to add a <code>LinkField</code> from the dataset to the result
	 * <code>Section</code>.
	 * 
	 * @param dataset
	 *            The current dataset.
		...
		try {
			...
			if (!StrUtils.isEmptyOrNullOrZero(labelKey) || element.getChild("field") != null) {
				...
				if (value != null) {
					...
					// TODO find better solution as the previous content may be
					// a section
					// If there are more than one field in this section add the
					// value (incl. separator) to the previous field
					if (result.getContent().isEmpty()) {
	...
	/**
	 * Procedure to add a <code>plain</code> from the dataset to the result
	 * <code>Section</code>.
	 * 
	 * @param dataset
	 *            The current dataset.
	...
	/**
	 * Retrieves the context names of the contexts that are referenced in
	...
	/**
	 * This function checks if a config file for the given type exists and
	...
	/**
	 * Function to load a xml category file from disk, build a document and put
	...
	/**
	 * Reads an include XML file and returns the contained
	 * <code>Elements</code>.
	 * 
	 * @param type
	...
	private List<Element> getElementsFromFile(final String type) {
		...
		try {
			...
			final Element rootElement = document.getRootElement();
			final Namespace namespace = rootElement.getNamespace();
			// the include element may be either a list of section, of context
			// or of facet elements
	...
	/**
	 * Internally used method to get the external field names from the XML
	 * documents. If no corresponding document is found <code>null</code> is
	 * returned. The method uses <code>getFields</code> to get the values from
	 * the document.
	 * 
	 * @param type
	...
	/**
	 * Recursive function adding all external field names of an
	...
	/**
	 * Internal function adding field names from contexts to the
	 * <code>result</code>.
	 * 
	 * @param element
		...
		for (final Element childElement : children) {
			if (childElement.getName().equals("contextSection")) {
				...
				for (final Element sectionChild : contextSectionChildren) {
					addContextFieldToList(sectionChild, parentType, result, context);
				}
			} else {
	...
	/**
	 * Internal function adding field names from fields to the
	 * <code>result</code>. Only names that don't start with the parent type
	 * name are considered. For example 'objektkeramik' is not listed in the
	 * results for parentType 'objekt'. The reason is that such fields are
	...
	/**
	 * Methode retrieves context-image-descriptor instances from
	 * xml-descriptions
	 * 
	 * @param type
	...
	/**
	 * This method replaces an include element in the DOM by the corresponding
	 * real element. The real element is fetched either from the cache of
	 * include elements or read from file and then added to the cached elements
	 * list.
	 * 
	 * @param include
	...
	/**
	 * This function checks if an include file for the given type exists and
	 * returns its filename.
	 * 
	 * @param type
	...
	/**
	 * Recursive method to find all include tags in an <code>Element</code> and
	...
	/**
	 * Replaces include elements with the elements from the corresponding XML
	 * include files.
	 * 
	...
	public List<JointContextDefinition> getJointContextualizers(String type) {
		...
		if (cachedContextualizers == null) {
			...
			for (Element context : contexts.getChildren("context", nameSpace)) {
				...
				defintion
						.setStandardCIDOCConnectionType(context.getChildText("StandardCIDOCConnectionType", nameSpace));
				...
				for (Element join : context.getChildren("join", nameSpace)) {
					defintion.addJoin(join.getAttributeValue("type"), join.getAttributeValue("connectFieldParent"),
