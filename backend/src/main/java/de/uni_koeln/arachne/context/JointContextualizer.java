 ...
 * database). but NOT based on the SemanticConnections table in the DB but by
...
public class JointContextualizer extends AbstractContextualizer {
	...
	public List<AbstractLink> retrieve(final Dataset parent) {
		...
		final List<Map<String, String>> contextContents = genericSQLDao.getConnectedEntitiesJoint(contextType,
				parent.getArachneId().getArachneEntityID(), jointContextDefinition);
	...
	private List<AbstractLink> retrieveGrouped(Dataset parent, List<Map<String, String>> contextContents) {
		...
		// results were sorted by sql BUT this got lost through grouping, so we
		// have to sort again
	...
	/**
	 * Creates a new dataset which is a context from the results of an SQL
	 * query.
	 * 
	 * @param map
