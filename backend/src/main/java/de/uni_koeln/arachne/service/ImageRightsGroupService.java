...
public class ImageRightsGroupService {
	...
	/**
	 * Constructor to initialize the image resolution parameters set in
	...
	/**
	 * Method to check, if an user has the rights to see an image at the specified
	 * resolution.
	 * 
	 ...
	 * @return A boolean value indicating if the user is allowed to see images at
	...
	public int getMaxResolution(final Dataset imageEntity, final ImageRightsGroup imageRightsGroup) {
		...
		// if user doesn't have group he is not allowed to view the image in any
		// resolution
		...
		// if override_for_group is set and the user has that exact group, the user is
		// allowed to view the image in high resolution
		if (!imageRightsGroup.getOverrideForGroup().isEmpty()
				&& currentUser.hasGroup(imageRightsGroup.getOverrideForGroup())) {
	...
	/**
	 * Convert database text fields to the corresponding ints as defined in
	 * application.properties.
	 * 
	...
	private int resolutionNameToInt(final String resolutionName) {
		...
		if ("THUMBNAIL".equals(resolutionName)) {
			return resolution_THUMBNAIL;
		}
		if ("PREVIEW".equals(resolutionName)) {
			return resolution_PREVIEW;
		}
		if ("HIGH".equals(resolutionName)) {
			return resolution_HIGH;
		}
		return -1;
	}
	...
	/**
	 * Get the filename of the watermark based on the user and an image rights
	 * group.
	 * 
	 * @param imageEntity      The dataset of the image.
	...
	public String getWatermarkFilename(final Dataset imageEntity, final ImageRightsGroup imageRightsGroup) {
		...
		// if override_for_group is set and the user has that exact group, the user is
		// allowed to view the image without watermark
		if (!imageRightsGroup.getOverrideForGroup().isEmpty()
				&& currentUser.hasGroup(imageRightsGroup.getOverrideForGroup())) {
