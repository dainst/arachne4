...
public class IIPService {
	...
	/**
	 * Constructor to initialize the image server parameters set in
	 * application.properties.
	 * 
	 * @param imageServerPath      The path of the image server.
	 * @param imageServerName      The name of the image server.
	 * @param imageServerExtension The extension (used for watermarking) of the
	 *                             image server
	 ...
	 * If the requested resolution equals 300 the image is loaded from the local
	 * cache directory. If the image isn't
	 * cached already it will be retrieved from the image server and stored in the
	 * cache directory.
	 * 
	 ...
	 * @return The requested image or <code>null</code> if no image could be
	 *         retrieved from the server.
	 */
	public TypeWithHTTPStatus<byte[]> getImage(final long entityId, final int requestedWidth,
	...
	/**
	 * Writes a JPEG image to the cache directory. The path of the image will be
	 * kept the same and be created if
	...
	/**
	 * Converts the given <code>imageName</code> to the corresponding name used for
	 * caching (in essence it replaces the
	 * 'ptif' extension with 'jpeg', prefixes the path with the cache directory and
	 * returns the result as path object).
	 * 
	 * @param imageName   The image name (*.ptif) including the path.
	 * @param watermarked
	...
	private Path imageNameToCachedImageName(String imageName, boolean watermarked) {
		...
		return Path.of(cachedImageName);
	}

	/**
	 * Here the real work for the <code>getDataForIIPViewer</code> is done. This
	 * method sends a HTTP-request to the image server and
	 * either gets the meta data or a tile of the requested image. If meta data is
	 * fetched it is returned. If an image tile is fetched
	 * it is written to the HTTP response output stream and <code>null</code> is
	 * returned.
	 * 
	 * @param entityId    The entity id of an image.
	 * @param queryString The full query string sent by an IIPImage client.
	 * @return Either the meta data of an image wrapped in a
	 *         <code>ResponseEntity</code> or <code>null</code>.
	 */
	@Deprecated
	...
	public TypeWithHTTPStatus<String> getImagePropertiesForZoomifyViewer(final long entityId) {
		...
		try {
			...
			final URL serverAdress = new URL(
					imageServerPath + imageServerInstance + imageServerExtension + queryString);
	...
	/**
	 * Method to retrieve the name of the image, the allowed maximum resolution and
	 * the watermark to use. Maximum
	 * resolution and watermark depend on the rights of the currently logged in
	 * user.
	 * 
	 * @param entityId            The unique image ID.
	 * @param requestedResolution The requested resolution of the image.
	 * @return An instance of <code>ImagePorperties</code> containing the name,
	 *         granted resolution, maximum resolution
	 *         and watermark of the requested image as well as an HTTP response code
	...
	public ImageProperties getImageProperties(final long entityId, final int requestedResolution) {
		...
		if (entityId > 0) {
			...
			// imageName == null means the user is not allowed to access the image dataset
			// in 'marbilder'
			...
			// Check image rights
			final ImageRightsGroup imageRightsGroup = imageRightsDao
					.findByName(imageEntity.getField("marbilder.BildrechteGruppe"));
	...
	/**
	 * Inner class to wrap multiple return values from the
	 * <code>getImageProperties</code> method.
	 */
