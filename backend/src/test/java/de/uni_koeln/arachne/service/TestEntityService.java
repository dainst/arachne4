...
public class TestEntityService {
	...
	@InjectMocks
	private EntityService entityService = new EntityService(false,
	...
	public void setUp() throws Exception {
		...
		when(esService.getDocumentFromCurrentIndex(0l, null,
				new String[] { "boost", "connectedEntities", "degree", "fields" }, LANG))
				.thenReturn(new TypeWithHTTPStatus<String>(HttpStatus.FORBIDDEN),
						new TypeWithHTTPStatus<String>("Test Doc", HttpStatus.OK));
	...
	public void testGetFormattedEntityByIdAsJsonString() throws Transl8Exception {
		// forbidden - neither has the user the correct datasetgroup nor is the
		// dataimport calling the method
		assertEquals("forbidden", entityService.getFormattedEntityByIdAsJsonString(testId, LANG));

		// No need to test the concrete return value here as it is the one we inject
		// into the ResponseFactory
