...
public class TestEntityController {
	...
	public void setUp() throws Exception {
		...
		when(entityService.getEntityFromIndex(1l, null, LANG))
				.thenReturn(new TypeWithHTTPStatus<String>("{test:value}"));
		when(entityService.getEntityFromIndex(1l, "test", LANG))
				.thenReturn(new TypeWithHTTPStatus<String>("{test:value}"));
		when(entityService.getEntityFromDB(1l, null, LANG)).thenReturn(new TypeWithHTTPStatus<String>("{test:value}"));
		when(entityService.getEntityFromDB(1l, "test", LANG))
				.thenReturn(new TypeWithHTTPStatus<String>("{test:value}"));
		// instance of Transl8Serive to be able to throw its exception
		Transl8Service dummyTransl8Service = new Transl8Service();
		when(entityService.getEntityFromIndex(2l, null, LANG))
				.thenThrow(dummyTransl8Service.new Transl8Exception("test Exception"));
		when(entityService.getEntityFromIndex(2l, "test", LANG))
				.thenThrow(dummyTransl8Service.new Transl8Exception("test Exception"));
		...
		when(imageService.getImagesSubList(eq(testDataset.getArachneId()), anyInt(), anyInt()))
				.thenAnswer(new Answer<TypeWithHTTPStatus<List<Image>>>() {
					...
					public TypeWithHTTPStatus<List<Image>> answer(InvocationOnMock invocation) throws Throwable {
						final int offset = invocation.getArgument(1, Integer.class);
						final int limit = invocation.getArgument(2, Integer.class);
	...
	public void testHandleImagesRequest() throws Exception {
		final String expectedJson = jsonMapper
				.writeValueAsString(new ImageListResponse(TestData.getTestDataset().getImages()));
		mockMvc.perform(
				get("/entity/1/images")
						.contentType(APPLICATION_JSON_UTF8))
				.andExpect(status().isOk())
