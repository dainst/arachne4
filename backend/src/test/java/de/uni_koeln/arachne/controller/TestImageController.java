...
public class TestImageController {
		...
		when(iipService.getImage(eq(0l), anyInt(), anyInt()))
				.then(new Answer<TypeWithHTTPStatus<byte[]>>() {
					...
					public TypeWithHTTPStatus<byte[]> answer(InvocationOnMock invocation)
							throws Throwable {
						int width = invocation.getArgument(1, int.class);
						int height = invocation.getArgument(2, int.class);

						TypeWithHTTPStatus<byte[]> result = new TypeWithHTTPStatus<byte[]>(
								TestData.getScaledTestImageJPEG(width, height));
						return result;
					}
				});
	...
	public void testGetWidthValidId() throws Exception {
		...
		BufferedImage resultImage = ImageIO
				.read(new ByteArrayInputStream(result.getResponse().getContentAsByteArray()));
