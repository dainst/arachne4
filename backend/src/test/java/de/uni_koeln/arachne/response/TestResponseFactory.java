...
public class TestResponseFactory { // NOPMD
	...
	@InjectMocks
	private final ResponseFactory responseFactory = new ResponseFactory("testAddress.com", new ArrayList<String>() {
		{
			add("facet_test");
		}
	}, 2);
	...
	public void setUp() throws Transl8Exception {
		...
		for (int i = 2; i < 6; i++) {
			final CatalogEntryInfo mockCatalogData = new CatalogEntryInfo(i,
					(String) mockCatalogDataList.get(i - 2).getPath(), i);
	...
	public void testSuggest() throws Transl8Exception {
		...
		assertTrue(response
				.contains("\"suggest\":{\"input\":[\"Title of the Test\",\"test facet value\"],\"weight\":146}"));
